...
def get_project_info(session, project_name=None, only_open_project=True, udfs=None):
    ...
    if udfs:
        q = q.filter(t.EntityUdfView.udfname.in_(udfs))
    return q.all()
...
def get_samples_and_processes(session, project_name=None, sample_name=None, list_process=None, workstatus=None, only_open_project=True):
    ...
    q = session.query(t.Project.name, t.Sample.name, t.ProcessType.displayname, t.Process.workstatus, t.Process.daterun) \
...
if __name__ == "__main__":
    ...
    session = get_session()
    #res = non_QC_queues(session, project_name='X16032')
    res = non_QC_queues(session, sample_name='X16137P001E01')
